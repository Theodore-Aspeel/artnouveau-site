...
**Dossier privé Google Drive créé pour ce livrable :** « ANAD 2.0 — Catalogue Instagram C1 — privé » (dans l’espace privé du compte, distinct du dépôt public). Le fichier de référence pour reprise est `ANAD-C1-catalogue-1431-prive-avec-scripts.zip` : il contient le catalogue complet CSV/JSON, le résumé, un index expurgé, `extract.py`, `classify.py`, `finalize.py` et les notes de méthode. **Ce ZIP est la version complète faisant foi**, notamment pour les clusters de légendes. SHA-256 du ZIP livré : `79bf2957f6e2764664ccc4475f651da9cb9b55651e3fead6e75164d80a454da6`. Les deux HTML sources et toutes les photographies restent dans leur dossier Drive source d’origine, sans copie dans ce ZIP.
...
Les rapports photographiques précédents vérifient directement plusieurs compositions exportées pour **neuf nouveaux sujets explicitement documentés** : Casina Cinese, Maison Spitzer, Maison Huot, Maison Losseau, Maison Bastin, Maison Piot, maison de la rue Warocqué, église Sainte-Thérèse de Wattrelos et Maison Paon de Gand. Le **Villino Florio** constitue un autre candidat à cinq publications, mais les contrôles visuels documentés dans les rapports C1 ne permettent pas, à cette étape, de le transformer en un dixième dossier *vérifié* : il reste candidat non validé visuellement. Cette distinction préserve la piste sans fabriquer une attestation manquante.
